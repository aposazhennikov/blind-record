...
// Reference: https://docs.espressif.com/projects/esp-idf/en/latest/esp32/api-reference/peripherals/adc_calibration.html
//
...
static bool adc_calibration_init_func(adc_unit_t unit,
                                       adc_atten_t atten,
  ...
  if (!calibrated) {
    ESP_LOGI(TAG, "calibration scheme version is %s",
             "Curve Fitting");
...
esp_err_t battery_init(void) {
  ...
  adc_calibration_init = adc_calibration_init_func(
      BATTERY_ADC_UNIT, BATTERY_ADC_ATTEN, &adc_cali_handle);
